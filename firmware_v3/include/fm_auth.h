...
/**
 * V3: generic form for any authenticated frame whose last FM_MAC_LEN bytes are
 * the MAC. The MAC covers byte 0 and bytes 2 .. len-FM_MAC_LEN-1, i.e. every
 * byte except hopLimit and the MAC itself. For a 22-byte alarm this is exactly
 * the region fmAuthSignAlarm() covers, so V3 alarms stay compatible with the
 * earlier firmware. Used for text messages.
 */
