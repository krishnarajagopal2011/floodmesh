...
#define FM_TYPE_TEXT        0x03   // V3: short text message, PSK-authenticated

#define FM_CALLSIGN_LEN     5      // on air: no NUL terminator
#define FM_CH_COUNT         4

#ifndef FM_HOP_DEFAULT
#define FM_HOP_DEFAULT      3      // enough for a colony; override for chain tests
#endif
...
enum {
  ...
  // --- V3 text: 14-byte header + 6-bit packed text + 8-byte MAC
  //   9   1  textLen   characters, 1..FM_TEXT_MAX_CHARS
  //   10  4  counter   anti-replay, same sequence as alarms
  //   14  P  text      ceil(textLen * 6 / 8) bytes, fm_text charset
  //   14+P 8 mac       HMAC over [0] + [2 .. 13+P] (hopLimit excluded)
  FM_OFF_TEXTLEN    = 9,
  FM_OFF_TCOUNTER   = 10,
  FM_OFF_TEXT       = 14,
};
...
#define FM_TEXT_MAX_CHARS   60
#define FM_TEXT_PACKED_MAX  ((FM_TEXT_MAX_CHARS * 6 + 7) / 8)          // 45
#define FM_TEXT_LEN_MAX     (FM_OFF_TEXT + FM_TEXT_PACKED_MAX + FM_MAC_LEN)   // 67
inline size_t fmTextPackedBytes(uint8_t chars) { return ((size_t)chars * 6 + 7) / 8; }
