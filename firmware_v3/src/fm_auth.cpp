...
// ---------------------------------------------------------------- V3 generic
namespace {
bool frameMac(const uint8_t *f, size_t len, uint8_t out[FM_MAC_LEN]) {
  if (!g_ready || len < (size_t)(3 + FM_MAC_LEN)) return false;
  uint8_t full[MBEDTLS_MD_MAX_SIZE];
  if (mbedtls_md_hmac_starts(&g_md, g_psk, FM_PSK_LEN) != 0 ||
      mbedtls_md_hmac_update(&g_md, f, 1) != 0 ||                          // ver+type
      mbedtls_md_hmac_update(&g_md, f + 2, len - 2 - FM_MAC_LEN) != 0 ||   // skip hop
      mbedtls_md_hmac_finish(&g_md, full) != 0) {
    return false;
  }
  memcpy(out, full, FM_MAC_LEN);
  return true;
}
}  // namespace

void fmAuthSignFrame(uint8_t *frame, size_t len) {
  if (frame == NULL || len < (size_t)(3 + FM_MAC_LEN)) return;
  uint8_t mac[FM_MAC_LEN];
  if (!frameMac(frame, len, mac)) {
    memset(frame + len - FM_MAC_LEN, 0, FM_MAC_LEN);
    return;
  }
  memcpy(frame + len - FM_MAC_LEN, mac, FM_MAC_LEN);
}

bool fmAuthVerifyFrame(const uint8_t *frame, size_t len) {
  if (frame == NULL || len < (size_t)(3 + FM_MAC_LEN)) return false;
  uint8_t mac[FM_MAC_LEN];
  if (!frameMac(frame, len, mac)) return false;
  uint8_t diff = 0;
  for (uint8_t i = 0; i < FM_MAC_LEN; i++) diff |= (uint8_t)(mac[i] ^ frame[len - FM_MAC_LEN + i]);
  return diff == 0;
}
