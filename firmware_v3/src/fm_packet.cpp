...
bool fmParseHeader(const uint8_t *buf, size_t len, FmHeader *out) {
  ...
  if (type != FM_TYPE_ALARM && type != FM_TYPE_VOICE && type != FM_TYPE_TEXT) return false;
  ...
  } else if (type == FM_TYPE_TEXT) {
    if (len < (size_t)(FM_OFF_TEXT + FM_MAC_LEN)) return false;
    const uint8_t n = buf[FM_OFF_TEXTLEN];
    if (n == 0 || n > FM_TEXT_MAX_CHARS) return false;
    // Exact length: the MAC sits at a position derived from textLen.
    if (len != (size_t)FM_OFF_TEXT + fmTextPackedBytes(n) + FM_MAC_LEN) return false;
  } else {
